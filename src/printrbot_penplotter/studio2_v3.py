...
(()=>{
  ...
  const staleMessages={source:'Generate grayscale to see the result.',threshold:'Generate black & white to see the result.',edges:'Generate edges to see the result.',style:'Generate style paths to see the result.'};
  const clearStageOutput=(id)=>{
    if(id==='source'){setStage('sourceCorrected','',staleMessages.source);}
    if(id==='threshold'){setStage('thresholdCorrected','',staleMessages.source);setStage('thresholdMask','',staleMessages.threshold);}
    if(id==='edges'){setStage('edgesMask','',staleMessages.threshold);setStage('edgesEdges','',staleMessages.edges);}
    if(id==='style'){setStage('styleEdges','',staleMessages.edges);setSvg('preview','',staleMessages.style);}
  };
  const markStale=(step)=>{for(const id of staleDependents[step]||[])if(stageData[id]){stageStale[id]=true;clearStageOutput(id);}};
  ...
  // Processing is explicit. Parameter edits mark the current output stale;
  // the user clicks that stage's Generate button when the settings are ready.
...
async def render_studio2_stage(request: Request) -> dict[str, object]:
    ...
    metadata.update(
        {
            "studio_schema": "printrbot-studio2/stage-v1",
...
async def render_studio2(request: Request) -> dict[str, object]:
    ...
    try:
        result = await legacy.render_studio2(
            ...
            style_min_stroke_length_px=_float(form, "style_min_stroke_length_px", 3.5),
            style_max_kept_strokes=_int(form, "style_max_kept_strokes", 0),
            one_line_bridge_distance_px=_float(form, "one_line_bridge_distance_px", 6.0),
