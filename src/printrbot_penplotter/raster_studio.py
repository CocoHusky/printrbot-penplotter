...
</style><style>.app-tabs{display:flex;gap:6px;margin:0 0 16px;padding:5px;background:#132231;border-radius:12px;width:max-content}.app-tabs a{color:#9db0c2;text-decoration:none;padding:8px 13px;border-radius:8px;font-weight:700;font-size:12px}.app-tabs a.active{background:#79d4ff;color:#091019}</style></head><body><main>
<nav class="app-tabs" aria-label="Printrbot tools"><a href="/">Write notes</a><a class="active" href="/raster">Image trace</a><a href="/studio2">Art workflow</a></nav>
...
<button id="trace" disabled>Trace / refresh</button>
<div class="editor-toolbar"><button id="undo" class="secondary" disabled>Undo</button><button id="delete" class="danger" disabled>Delete</button><button id="reverse" class="secondary" disabled>Reverse</button><button id="split" class="secondary" disabled>Split</button><button id="join" class="secondary" disabled>Join 2</button></div>
<button id="finalize" disabled>Finalize edited paths</button><button id="downloadG" class="secondary" disabled>Download G-code</button><button id="downloadSvg" class="secondary" disabled>Download edited SVG</button><button id="downloadJob" class="secondary" disabled>Download job JSON</button>
<div id="status" class="status" role="status" aria-live="polite">Choose an image, then click Trace / refresh.</div><pre id="meta"></pre>
...
const drop=$('drop'),file=$('file');drop.onclick=()=>file.click();file.onchange=()=>{chosen=file.files[0]||null;resetForNewImage()};
...
drop.addEventListener('drop',e=>{chosen=e.dataTransfer.files[0]||null;resetForNewImage()});
function resetForNewImage(){polys=[];history=[];selected.clear();latestG='';sidecar=null;$('trace').disabled=!chosen;for(const id of ['undo','delete','reverse','split','join','finalize','downloadG','downloadSvg','downloadJob'])$(id).disabled=true;$('original').innerHTML='';$('mask').innerHTML='';$('editor').innerHTML='';$('final').innerHTML='';$('meta').textContent='';$('status').textContent=chosen?'Image loaded. Click Trace / refresh to process it.':'Choose an image, then click Trace / refresh.'}
...
function syncEditButtons(){ $('undo').disabled=!history.length; $('delete').disabled=!selected.size; $('reverse').disabled=!selected.size; $('split').disabled=selected.size!==1; $('join').disabled=selected.size!==2; }
function renderEditor(){const good=polys.map((p,i)=>[p,i]).filter(([p])=>p&&p.length>1);if(!good.length){$('editor').innerHTML='No paths';syncEditButtons();return}const [x0,y0,x1,y1]=bounds(),pad=4,w=Math.max(1,x1-x0),h=Math.max(1,y1-y0);let s=`<svg id="editSvg" viewBox="${x0-pad} ${-y1-pad} ${w+2*pad} ${h+2*pad}">`;
...
s+='</svg>';$('editor').innerHTML=s;document.querySelectorAll('#editSvg polyline').forEach(el=>el.onclick=e=>{const i=Number(el.dataset.i);if(!e.shiftKey)selected.clear();selected.has(i)?selected.delete(i):selected.add(i);renderEditor()});setupDrag();syncEditButtons()}
