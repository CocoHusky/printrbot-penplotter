...
<div class="row"><div><label>Minimum contour length (px)</label><input name="style_min_stroke_length_px" type="number" min="0" max="1000" step="0.25" value="8"></div><div><label>Keep longest contours (0 = all)</label><input name="style_max_kept_strokes" type="number" min="0" max="200000" step="1" value="250"></div></div>
<label>One-line bridge distance (px)</label><input name="one_line_bridge_distance_px" type="number" min="0" max="20" step="0.5" value="0"><div class="hint">Fast line art does not bridge blank paper. Keep this at 0 unless you deliberately want nearby endpoints connected.</div>
<div class="hint">Fast defaults discard tiny traces and keep the longest contours. Raise the limits only when you need more detail.</div>
</div>
...
<label>Max plotted lines (longest kept)</label><input name="plot_stroke_limit" type="number" min="1" max="200000" value="250"><div class="hint">Fast default keeps the longest 250 paths. Shorter lines are dropped first to reduce pen travel and Z lifts.</div>
...
const fastLineStyles=[
  {value:'minimal_outline',label:'Minimal outline — fastest'},
  {value:'clean_outline',label:'Clean outline — recommended'},
  {value:'continuous_contour',label:'Continuous contour — fewer lifts'},
];
...
const fastShadingStyles=[
  {value:'parallel_hatch',label:'Parallel hatch — fastest fill'},
  {value:'crosshatch',label:'Crosshatch — recommended'},
  {value:'curved_hatch',label:'Curved hatch — softer shading'},
  {value:'contour_hatch',label:'Contour hatch — follows form'},
];
...
function options(select,values,current){select.innerHTML='';for(const item of values){const value=typeof item==='string'?item:item.value;const label=typeof item==='string'?value.replaceAll('_',' '):item.label;const o=document.createElement('option');o.value=value;o.textContent=label;if(value===current)o.selected=true;select.appendChild(o);}}
...
setupStageTabs();
function updatePipeline(){const m=mode.value;if(m==='auto'){options(style,['Auto chooses after analysis'],'Auto chooses after analysis');style.disabled=true;style.name='';styleHint.textContent='Auto ranks compatible recipes and renders the winner. Limits still apply to the selected recipe.';lineArtAdvanced.classList.add('hidden');shadingAdvanced.classList.add('hidden');geometryLimits.classList.remove('hidden');}else if(m==='line_art'){const current=fastLineStyles.some(item=>item.value===style.value)?style.value:'clean_outline';style.disabled=false;style.name='style';options(style,fastLineStyles,current);styleHint.textContent='Fast line art: longer, simpler contours only. Tiny traces are dropped; use Pen shading for hatch and fill.';lineArtAdvanced.classList.remove('hidden');shadingAdvanced.classList.add('hidden');geometryLimits.classList.remove('hidden');}else{const current=fastShadingStyles.some(item=>item.value===style.value)?style.value:'crosshatch';style.disabled=false;style.name='style';options(style,fastShadingStyles,current);styleHint.textContent='Reliable pen shading: four controlled fill recipes. Dense, dotted, and texture experiments are kept out of the normal workflow.';lineArtAdvanced.classList.add('hidden');shadingAdvanced.classList.remove('hidden');geometryLimits.classList.remove('hidden');}syncControlSections();}
