...
void test_sequence_accepts_guarded_generated_plot() {
  JobValidationState state;
  const char* lines[] = {
      "G21", "G90", "M400", "G28", "M400",
      "G0 Z5 F120", "G0 X20 Y20 F1000", "G0 Z0 F120",
      "G1 X30 Y30 F600", "G0 Z5 F120", "M400"};
...
void test_sequence_rejects_simultaneous_xy_and_z_motion() {
  JobValidationState state;
  TEST_ASSERT_TRUE(validateJobSequenceLine("G21", state).accepted);
  TEST_ASSERT_TRUE(validateJobSequenceLine("G90", state).accepted);
  TEST_ASSERT_TRUE(validateJobSequenceLine("G28", state).accepted);
  const auto result = validateJobSequenceLine("G0 X10 Y10 Z5 F120", state);
  TEST_ASSERT_FALSE(result.accepted);
...
int main(int, char**) {
  ...
  RUN_TEST(test_sequence_accepts_guarded_generated_plot);
  ...
  RUN_TEST(test_sequence_rejects_simultaneous_xy_and_z_motion);
  return UNITY_END();
}
