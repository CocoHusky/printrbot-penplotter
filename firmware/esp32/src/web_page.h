...
<details><summary>Editable G-code components</summary><p class="small">Pen-down strokes are selectable components. Select one or more, then move, rotate, duplicate, delete, undo, or redo. Added components are parsed from the G-code box below.</p><div class="component-editor"><div><label for="componentSelect">Select components</label><select id="componentSelect" multiple></select><div class="buttons"><button class="secondary" onclick="selectAllComponents()">Select all</button><button class="secondary" onclick="deleteSelectedComponents()">Delete selected</button></div></div><div><div class="offset-grid"><div><label for="componentDx">Move X (mm)</label><input id="componentDx" type="number" step="0.1" value="0"></div><div><label for="componentDy">Move Y (mm)</label><input id="componentDy" type="number" step="0.1" value="0"></div><div><label for="componentRotation">Rotate (degrees)</label><input id="componentRotation" type="number" step="1" value="0"></div></div><div class="buttons"><button class="secondary" onclick="transformSelectedComponents()">Move / rotate selected</button><button class="secondary" onclick="duplicateSelectedComponents()">Duplicate selected</button><button class="secondary" onclick="undoEditor()">Undo</button><button class="secondary" onclick="redoEditor()">Redo</button></div><label for="componentGcode">Add G-code component(s)</label><textarea id="componentGcode" placeholder="Paste G0/G1 pen strokes here"></textarea><button class="secondary" onclick="addGcodeComponents()">Add G-code component(s)</button></div></div></details>
...
const BED={xmin:0,xmax:152.4,ymin:0,ymax:152.4,margin:8};let components=[],selectedComponents=[],editorUndo=[],editorRedo=[];
...
function loadEditorFromText(text){components=parseEditableComponents(text);selectedComponents=[];editorUndo=[];editorRedo=[];refreshComponentList()}
