...
def test_notes_workspace_has_simple_lettering_choices() -> None:
    ...
    assert "Handwriting uses Graves neural centerline strokes automatically" in text
    assert 'id="neuralExperimental"' not in text
...
def test_robot_mode_honors_selected_centerline_font() -> None:
    response = client.post(
        "/api/render",
        json={"text": "Mixed case", "preset": "robot", "stroke_font": "hershey-script"},
    )
    assert response.status_code == 200
    assert response.json()["metadata"]["stroke_font"] == "Hershey Script"
